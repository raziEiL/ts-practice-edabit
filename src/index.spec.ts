...
expect().toEqual();
    ...
    test("colorConversion", () => {
        expect(Edabit.colorConversion({ r: 126, g: 214, b: 131 })).toBe("#7ed683");
        expect(Edabit.colorConversion({ r: 255, g: 255, b: 255 })).toBe("#ffffff");
        expect(Edabit.colorConversion({ r: 0, g: 0, b: 0 })).toBe("#000000");
        expect(Edabit.colorConversion({ r: 3, g: 1, b: 200 })).toBe("#0301c8");
        expect(Edabit.colorConversion({ r: 256, g: 0, b: 0 })).toBe("Not valid input");
        expect(Edabit.colorConversion({ r: 0, g: 256, b: 0 })).toBe("Not valid input");
        expect(Edabit.colorConversion({ r: 0, g: 0, b: 256 })).toBe("Not valid input");
        expect(Edabit.colorConversion({ r: -1, g: 0, b: 0 })).toBe("Not valid input");
        expect(Edabit.colorConversion({ r: 0, g: -1, b: 0 })).toBe("Not valid input");
        expect(Edabit.colorConversion({ r: 0, g: 0, b: -1 })).toBe("Not valid input");
        expect(Edabit.colorConversion("fffffff")).toBe("Not valid input");
        expect(Edabit.colorConversion("#fffffff")).toBe("Not valid input");
    });
    test("stringFret", () => {
        expect(Edabit.stringFret(2, 10)).toBe("A");
        expect(Edabit.stringFret(6, 3)).toBe("G");
        expect(Edabit.stringFret(1, 18)).toBe("A#/Bb");
        expect(Edabit.stringFret(0, 16)).toBe("Invalid input");
        expect(Edabit.stringFret(3, 1)).toBe("G#/Ab");
        expect(Edabit.stringFret(5, 3)).toBe("C");
        expect(Edabit.stringFret(5, 25)).toBe("Invalid input");
        expect(Edabit.stringFret(6, 13)).toBe("F");
        expect(Edabit.stringFret(4, 18)).toBe("G#/Ab");
        expect(Edabit.stringFret(5, 0)).toBe("A");
        expect(Edabit.stringFret(4, 6)).toBe("G#/Ab");
        expect(Edabit.stringFret(4, 20)).toBe("A#/Bb");
        expect(Edabit.stringFret(7, 10)).toBe("Invalid input");
        expect(Edabit.stringFret(5, 0)).toBe("A");
        expect(Edabit.stringFret(3, 14)).toBe("A");
        expect(Edabit.stringFret(2, 19)).toBe("F#/Gb");
        expect(Edabit.stringFret(4, 5)).toBe("G");
        expect(Edabit.stringFret(6, 10)).toBe("D");
        expect(Edabit.stringFret(2, 10)).toBe("A");
        expect(Edabit.stringFret(3, 0)).toBe("G");
    });
    test("getHashTags", () => {
        expect(Edabit.getHashTags("How the Avocado Became the Fruit of the Global Trade")).toEqual(["#avocado", "#became", "#global"]);
        expect(Edabit.getHashTags("Why You Will Probably Pay More for Your Christmas Tree This Year")).toEqual(["#christmas", "#probably", "#will"]);
        expect(Edabit.getHashTags("Hey Parents, Surprise, Fruit Juice Is Not Fruit")).toEqual(["#surprise", "#parents", "#fruit"]);
        expect(Edabit.getHashTags("Visualizing Science")).toEqual(["#visualizing", "#science"]);
    ...
    test("sortContacts", () => {
        ...
        expect(Edabit.sortContacts(["Al Gore", "Barack Obama"], "ASC")).toEqual(["Al Gore", "Barack Obama"]);
        expect(Edabit.sortContacts(["Albert Einstein"], "ASC")).toEqual(["Albert Einstein"]);
